...
if(isset($_GET['idc'])&&isset($_GET['idp'])&&isset($_GET['idr'])){
    $rowP = $p->getById($_GET['idp']);
    $rowR = $receipt->getById($_GET['idr']);
    $rowC = $c->getById($_GET['idc']);
}
?>
<!DOCTYPE html>
...
<div class="container-fluid">
    ...
    <div class="col-sm-9">
        ...
        <div class="alert alert-success alert-dismissible" role="alert">
            <div class="panel-title"><h3>Sửa hồ sơ</h3></div>
        ...
        <div >
            ...
            <div class="card-body">
                ...
                <form action=<?="http://localhost/TTGT/controller/edit.php?idc=".$_GET['idc']."&idp=".$_GET['idp']."&idr=".$_GET['idr']?> class="border border-light p-5" method="post">

                    <p class="h4 mb-4 text-center panel-title">Hồ sơ</p>
                    <div class="form-group row">
                        <div class="col-xs-4">
                            <label for="ex1">Họ và Tên</label>
                            <input class="form-control" id="ex1" type="text" name="hovaten" value="<?=$rowP['name'];?>" required>
                        </div>
                        <div class="col-xs-4">
                            <label for="ex2">Địa chỉ</label>
                            <input class="form-control" id="ex2" type="text" name="diachi" value="<?=$rowP['address'];?>" required>
                        </div>
                        <div class="col-xs-4">
                            <label for="ex3">Biển kiểm soát</label>
                            <input class="form-control" id="ex3" type="text" name="bienkiemsoat" value="<?=$rowC['bks'];?>" required>
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-xs-4">
                            <label for="ex4">Số quyết định xử phạt</label>
                            <input class="form-control" id="ex4" type="text" name="soquyetdinh" value="<?=$rowR['numberDecide'];?>"  required>
                        </div>
                        <div class="col-xs-4">
                            <label for="ex5">Ngày quyết định xử phạt</label>
                            <input id="datepicker" width="276" class="form-control" id="ex5" type="date" name="ngayqd" value="<?=$rowR['dayReceipt'];?>" required>
                        </div>
                        <div class="col-xs-4">
                            <label for="ex6">Ký hiệu biên lai</label>
                            <input class="form-control" id="ex6" type="text" name="kyhieubienlai" value="<?=$rowR['khReceipt'];?>" required>
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-xs-4">
                            <label for="ex7">Số biên lai</label>
                            <input class="form-control" id="ex7" type="text" name="sobienlai" value="<?=$rowR['numberReceipt'];?>" required>
                        </div>
                        <div class="col-xs-4">
                            <label for="ex8">Tiền phạt Lái xe</label>
                            <input  id="tienlx"  class="form-control" id="ex8" type="text" name="tienphatlaixe" value="<?=$rowR['employee'];?>" required>
                        </div>
                        <div class="col-xs-4">
                            <label for="ex9">Tiền phạt Chủ xe</label>
                            <input  id="tiencx"  class="form-control" id="ex9" type="text" name="tienphatchuxe" value="<?=$rowR['boss'];?>" required>
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-xs-4">
                            <label for="ex10">Tiền phạt nộp chậm</label>
                            <input id="tiennopcham"  class="form-control" id="ex10" type="text" name="tienphatnopcham" value="<?=$rowR['latePay'];?>" required>
                        </div>
                        <div class="col-xs-4">
                            <label for="ex11">Ngày nộp</label>
                            <input id="datepicker1" width="276" class="form-control" id="ex11" type="date" name="ngaynop" value="<?=$rowR['paymentDate'];?>" required>
                        </div>
                    </div>
                    <div class="form-group row">
                        <div class="col-xs-6">
                            <label for="textarea">Nội dung vi phạm</label>
                            <textarea id="textarea" class="form-control mb-4"  name="noidungvipham"  required><?=$rowR['description'];?></textarea>
                        </div>

                    </div>
                    <div class="form-group row">
                        <div class="col-xs-4">
                            <button class="btn btn-info btn-block my-4" type="submit" name="editReceipt">Thêm</button>
                        </div>
                        <div class="col-xs-4">
...
<script>
    var input = document.getElementById('tiencx');
    input.oninvalid = function(event) {
        event.target.setCustomValidity('Vui lòng nhập số tiền là số');
    }
    var input = document.getElementById('tienlx');
    input.oninvalid = function(event) {
        event.target.setCustomValidity('Vui lòng nhập số tiền là số');
    }
    var input = document.getElementById('tiennopcham');
    input.oninvalid = function(event) {
        event.target.setCustomValidity('Vui lòng nhập số tiền là số');
    }
